...
private:
	...
	void RobotInit()
	{
		...
		servo = new Servo(5); //not necessary
	}
	...
	void followBlock() //change servo into drive code, and it should work well :)
	{
		if(blocks->signature == 1) //if pixy identify object 1
		{
			...
			else if(servo->GetAngle() == 0)
			{
				sign = 1;
			}*/

			servo->SetAngle(blocks->x * .52);
			/*
			if(blocks->x > 160) //if the object is on the right of the pixy (160 is about the middle)
			{
				servo->Set(180.0*((150.0 - blocks->x)/150.0)); // servo goes to the left -- this may vary depending on the servo
			}
